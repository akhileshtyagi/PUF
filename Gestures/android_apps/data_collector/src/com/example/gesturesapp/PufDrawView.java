...
{
    public static final String DATA_FOLDER = "trade_data";
    /* only accept traces with a greater number of points than this */
    public static final int MIN_TRACE_LENGTH = 14;
        ...
        switch( me.getAction() )
        {
            case MotionEvent.ACTION_DOWN:
                ...
                response.add(new Point(me.getX(), me.getY(), me.getPressure(),
                        me.getSize(), me.getEventTime() - me.getDownTime()));
                break;
            case MotionEvent.ACTION_UP:
                tv.setText( "No touch detected." );

                // if the trace was long enough
                if(response.size() > MIN_TRACE_LENGTH) {
                ...
                // if the trace was not long enough
                // then nothing needs to be done
                // the response list will be cleared on action down
    ...
    public void writeResponseCsv()
    {
        ...
        String deviceName = prefs.getString("DeviceName", "nexus-200");
        String currentSeed = prefs.getString("CurrSeed", "1");

        File baseDir = new File(Environment.getExternalStorageDirectory(), DATA_FOLDER);
        if (!baseDir.exists())
        {
            baseDir.mkdirs();
        }
        ...
        try
        {
            ...
            String[] headers = { "X", "Y", "PRESSURE", "SIZE", "TIME" };
            ...
            for( int i = 0; i < response.size(); i++)
            {
                ...
                String[] row = { Float.toString(point.x),
                    ...
                    Float.toString(point.size),
                    Long.toString(point.time)};
                csvWrite.writeNext(row);
            }
            csvWrite.close();
        }
